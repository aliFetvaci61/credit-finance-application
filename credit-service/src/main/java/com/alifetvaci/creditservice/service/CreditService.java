...
public class CreditService {
    ...
    public void createCredit(CreditCreateRequest request, String identificationNumber){
        ...
        for (int i = 1; i <= request.getInstallmentCount(); i++){
            ...
            installmentRepository.save(Installment.builder().credit(credit).amount(installmentAmount).dueDate(LocalDateTime.ofInstant(calendar.toInstant(), ZoneId.systemDefault())).status(InstallmentStatus.UNPAID).build());
